...
// THE HACK: stay out 80 steps longer than dummy (120 instead of 200)
...
+pos(X,Y,_) : cell(X,Y,gold) & carrying_gold(N) & N < 3 // container has space
  <- do(pick);
     -+back_pos(X,Y). // remembers a place to return

// in a cell besides
+pos(X,Y,_) : cell(GX,GY,gold) & carrying_gold(N) & N < 3 // container has space
...
+pos(X,Y,_) : back_pos(BX,BY) & jia.direction(X, Y, BX, BY, D) // one step towards back_pos
  <- do(D).
