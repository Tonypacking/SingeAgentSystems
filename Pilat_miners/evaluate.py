...
ALL_GOLD_RE = re.compile(
    r"All golds collected in (\d+) cycles! Result \(red x blue\) = (\d+) x (\d+)"
)
STDOUT_RESULT_RE = re.compile(
    r"MATCH_RESULT reason=(\S+) world=(\d+) step=(\d+) red=(\d+) blue=(\d+)"
)
...
def make_mas2j(
    path: Path, world: int, red: str, blue: str, sleep_ms: int, visualize: bool
) -> None:
    red_block = textwrap.indent(
        textwrap.dedent(STRATEGIES[red]["agents"]).strip(), "        "
    )
    blue_block = textwrap.indent(
        textwrap.dedent(STRATEGIES[blue]["agents"]).strip(), "        "
    )
...
def run_match(
    world: int, red: str, blue: str, sleep_ms: int, timeout_s: int, visualize: bool
) -> dict[str, object]:
    ...
    with stdout_log.open("w", encoding="utf-8") as out:
        proc = subprocess.Popen(
            cmd, cwd=PROJECT_ROOT, stdout=out, stderr=subprocess.STDOUT
        )
        deadline = time.time() + timeout_s
        while time.time() < deadline:
            parsed_result = parse_stdout_result(stdout_log) or parse_match_log(
                match_log
            )
        ...
        if parsed_result is None:
            ...
            proc.wait(timeout=5)
            raise RuntimeError(
                f"Match timed out without a final score. See {match_log} and {stdout_log}"
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Evaluate gold miners strategies headlessly."
    )
    ...
    parser.add_argument(
        "--csv", type=Path, default=PROJECT_ROOT / "evaluation-results.csv"
    )
    parser.add_argument(
        "--visualize",
        action="store_true",
    ...
    for world in args.worlds:
        for run in range(1, args.runs + 1):
            print(
                f"Running world {world}, match {run}/{args.runs}: {args.red} vs {args.blue}",
                flush=True,
            )
            result = run_match(
                world,
